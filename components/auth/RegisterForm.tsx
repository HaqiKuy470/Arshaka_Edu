...
export default function RegisterForm({
  ...
  const strengthLabel = ['', 'Terlalu pendek', 'Cukup kuat', 'Kuat! 🔒'];
  ...
  return (
    <div className="space-y-5">
      ...
      <form id="form-register" onSubmit={handleRegister} className="space-y-4">
                ...
                {[1, 2, 3].map((i) => (
                  <div
                    ...
                    className={`flex-1 rounded-full transition-all duration-300 ${
                      strength >= i ? strengthColor[strength] : 'bg-zinc-800'
                    }`}
                  />
                ))}
              </div>
        ...
        <div className="space-y-1.5">
          <label htmlFor="register-grade" className="block text-sm font-medium text-zinc-300">
            Jenjang Pendidikan <span className="text-zinc-600">(opsional)</span>
          </label>
          <select
            id="register-grade"
            value={grade}
            onChange={(e) => setGrade(e.target.value)}
            className="w-full bg-zinc-900 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500/50 transition-all appearance-none cursor-pointer"
          >
            <option value="" className="bg-zinc-900">Pilih jenjang...</option>
